...
{
    public class S39P4Handle : IP4Handler
    {
        public string Hnalder(int row, ProcessContext context)
        {
            try
            {
                if (row < 1)
                    return null;
                var p38Dic = context.GetP4RowResult(38, row - 1).CheckIntValue();
                var max3Num = 0;
                ...
                var mark = 0;
                p38Dic = context.GetP4RowResult(38, row).CheckIntValue();
                ...
                context.SetP4Value(39, row, 0, mark);
                return null;
            }
            catch (Exception e)
            {
