...
{
    public class P2S19Handler:IP2Handler
    {
        ...
        public string Handler(int stage, int row, ProcessContext context)
        {
            ...
            if (Ying.Contains(source[0]))
            {
                ...
                {
                    context.SetP2Value(stage, 19, row, 0, true);
                }
                else
                {
                    context.SetP2Value(stage, 19, row, 1, true);
                }
            }
            else
            {
                if (Ying.Contains(source[1]))
                {
                    context.SetP2Value(stage, 19, row, 2, true);
                }
                else
                {
                    context.SetP2Value(stage, 19, row, 3, true);
                }
            }
