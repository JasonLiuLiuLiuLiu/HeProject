...
{
    public class S8P3Handler : IP3Handler
    {
        public string Hnalder(int row, ProcessContext context)
        {
            var p6Result = context.GetP3RowResult(6, row);
            ...
            context.SetP3Value(8, row, 0, c1);
            context.SetP3Value(8, row, 1, c2);
            context.SetP3Value(8, row, 2, c3);
            return null;
        }
    }
}